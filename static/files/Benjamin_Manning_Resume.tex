...
\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=7308598}{\bf How AI Prompts Can Teach Us About the Structure of Human Behavior}\\
...
We assign a large language model a ``type vector'' and prompt it to choose actions across settings for which we observe human choices.
For instance, the type vector $(2,4)$ might yield the prompt ``You are a $\frac{2}{5}$ in Altruism and $\frac{4}{5}$ in Trust.''
We vary the dimensions (e.g., Altruism, Fairness, Trust, $\dots$) and their values (e.g., 1--5) to identify the type space that best matches human choices.
Applying the method to 119,147 decisions made by 78,657 subjects from more than 35 countries across ten classic economic game roles, we find that human behavior can be closely matched using three dimensions: Risk Aversion, Strategic Sophistication, and Trust.
...
The results suggest that behavior across diverse settings can be approximated by a low-dimensional, portable representation, and that the method may be useful for studying other aspects of human behavior.
\end{singlespace}
