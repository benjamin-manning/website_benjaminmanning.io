...
We provide these type vectors to an LLM, which is then asked to choose actions for each setting.
...
By varying the type vectors assigned to the LLM and comparing its choices in various settings with those of people, we can identify an empirical upper bound on the dimensionality needed to approximate the data and infer individual-level types that can be used to predict each person's behavior in novel settings.
We apply the method to decisions made by 68,779 subjects in ten classic economic games.
...
When we infer types for individual subjects on a subset of the games, the types cluster into just a dozen groups, and we use them to predict individual behavior in a held-out game.
These predictions perform comparably to---and sometimes outperform---a flexible machine-learning benchmark, even though the benchmark is trained directly on the held-out game.
