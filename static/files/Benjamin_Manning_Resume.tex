...
B.A. Mathematics (College Honors in Arts \& Sciences) \hfill 2017
...
\section{\sc Grants (\$488,325)}
Google Cloud Research Credits Grant (\$5,000; w/ Alex Moehring) \hfill 2026 \\
...
\footnotesize
\textbf{Abstract:}
We present an approach for automatically generating and testing, \emph{in silico}, social scientific hypotheses.
...
\href{https://arxiv.org/abs/2608.18265}{\bf Modeling the Structure of Human Behavior with AI Prompt Vectors}\\
...
\textit{Under Review at Nature}

\begin{singlespace}
\footnotesize
\textbf{Abstract:}
We introduce a general, easy-to-implement AI-based method for modeling and analyzing the structure and complexity of human behavior.
We assign a large language model a ``type vector'' and then prompt it to choose actions across settings in which we observe human choices.
For instance, the type vector $(2,4)$ becomes ``You are a player characterized by the following profile: Altruism: 2 out of 5, Risk Aversion: 4 out of 5,'' after which it is prompted to make choices.
We vary the dimensions (e.g., Altruism, Fairness, Trust, $\dots$) and values (e.g., 1--5) to minimize distance to human choices.
Applying the method to 119,147 decisions made by 78,657 subjects from more than 35 countries across 10 classic economic game roles, we find that human behavior can be closely matched using three dimensions: Risk Aversion, Strategic Sophistication, and Trust.
Moreover, the types needed to fit individuals across games cluster into fewer than a dozen groups, and can predict behavior in held-out games with different rules and available actions.
The results suggest that behavior across diverse settings can be approximated by a low-dimensional, portable representation, supporting the possibility of general yet parsimonious theories across the behavioral sciences.
More broadly, this new modeling method can provide insights into the structure of many human behaviors.
\end{singlespace}
...
\begin{singlespace}
\footnotesize
\textbf{Preliminary Abstract (subject to change):}
...
{\bf When Stated, Revealed, and Normative Preferences Diverge: Experimental Evidence from Personalized News Feeds}\\
...
\textit{The Review of Economics and Statistics,} Conditionally Accepted
\vspace{0.25cm}
\begin{itemize}
...
\footnotesize
\textbf{Abstract:}
We argue that newly-developed large language models (LLMs), because of how they are trained and designed, are implicit computational models of humans---a \emph{Homo silicus}.
...
Chapter 6 in \textit{The Economics of Transformative AI,} University of Chicago Press, 2026
\vspace{0.25cm}
\begin{itemize}
...
\textbf{2026 / 2027 (Includes scheduled talks):} 
University of North Dakota (Real Estate, Economics \& Finance);
...
\textbf{2026 / 2027 (Includes scheduled talks):}
Zurich Workshop on Economics and Psychology;
