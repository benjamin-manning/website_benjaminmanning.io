...
class SocialInteraction(RegisteredSerializable, PromptMixin): 
    ...
    def ask_oracle_post(self, items):
        ...
        while True:
            agent_thoughts = {}
            ## Ask agents for their thoughts
            for agent in self.agents:
                agent_thoughts[agent["name"]] = ...
            prompt_params = { 
            "scenario": self.scenario,
            "statements": self.statements,
            "agent_list": agent_list,
            ...
            ask_template = self.generate_prompt("ask_oracle_post.txt", template_dir = self.template_dir, **prompt_params)
